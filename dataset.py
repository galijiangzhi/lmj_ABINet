...
class ImageDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 charset_path='data/tibetan_charset.txt', #字符集文件的路径。该文件包含了数据集中可能出现的字
        ...
        self.charset = CharsetMapper(charset_path, max_length=max_length + 1)  #调用utile.py里的charsetmapper类构造字典
    ...
    def get(self, idx):
        ...
        with self.env.begin(write=False) as txn:
            ...
            try:
                #baizhen——120
                #༄
                label = str(txn.get(label_key.encode()), 'utf-8').strip('').replace(' ','').replace('་','').replace('།','').replace('ུ','') # 从数据库中根据label_key的键取出信息 label
                print(f"baizhen _______________________________ label={label}")
                print(f'baizhen!!!!!!!!!!!!!!!!!!!!!!!!lebel == {label}')
                #baizhen

                print(f'baizhen++++++++++++++++++++++++++{self.character}')
